...
const cancelLike = async (item) => {
    try {
        let res;
        if (item.source === 'admin') {
            // 管理员文章
            res = await interactArticle(item.articleId, 1);
        }
        else {
            // 用户文章
            res = await interactUserArticle(item.articleId, 1);
        }
        if (res.code === 200) {
...
/** @type {__VLS_StyleScopedClasses['my-home-likes']} */ ;
...
for (const [item] of __VLS_vFor((__VLS_ctx.likes))) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ onClick: (...[$event]) => {
                ...
                // @ts-ignore
                [vLoading, loading, loading, likes, likes, viewArticle,];
            } },
        key: (item.articleId),
        ...{ class: "article-item" },
    });
